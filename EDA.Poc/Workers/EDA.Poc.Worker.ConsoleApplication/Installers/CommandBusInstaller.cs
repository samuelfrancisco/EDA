...
using EDA.Poc.Pagamentos.CommandHandlers;
using GreenPipes;
using MassTransit;
using System;
            ...
            var commandBusControl = Bus.Factory.CreateUsingRabbitMq(cfg =>
                                                                        {
                                                                            ...
                                                                            cfg.ReceiveEndpoint(host, "commands_queue", ep =>
                                                                                                                            {
                                                                                                                                ep.Consumer(typeof(ComprarConteudoHandler), container.Resolve);

                                                                                                                                ep.Consumer(typeof(GerarCobrancaDaCompraParaOClienteHandler), container.Resolve);

                                                                                                                                ep.Consumer(typeof(RegistrarLicencaParaClienteHandler), container.Resolve);

                                                                                                                                ep.Consumer(typeof(ConcluirOrdemDeCompraHandler), container.Resolve);

                                                                                                                                ep.Consumer(typeof(AvaliarConteudoHandler), container.Resolve);

                                                                                                                                ep.Consumer(typeof(RegistrarDebitoRecorrenteNaFaturaDoClienteHandler), container.Resolve);
                                                                                                                            });                                                                            
